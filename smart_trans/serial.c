        ...
        if(rxBufferSize >= HEADER_SIZE) {
            ...
            if(rxBufferSize == size + HEADER_SIZE) {
                ...
                switch (mainHeader)
                {
                    ...
                    {
                        ValueHeader_t valueHeader = (ValueHeader_t)uartRxBuffer[3];
                        if(valueHeader == PERIOD) {
                            setBLEBeaconTransmitPeriod(uartRxBuffer[4]);
                            unsigned char data[] = {ACK, 0, 0};
                            send_array(data, sizeof(data));
                        }
                        else if(valueHeader == OUTPUT_POWER) {
                            TransmitterOutputPower_t transmitterOutputPower = (TransmitterOutputPower_t)uartRxBuffer[4];
                        ...
                        else if(valueHeader == PAYLOAD) {
                            setBLEBeaconPayload(&uartRxBuffer[4], size - 2);
                            unsigned char data[] = {ACK, 0, 0};
                            send_array(data, sizeof(data));
                        }
                    }
